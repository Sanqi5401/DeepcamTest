package com.deepcam.access.view;
...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] bytes, Camera camera) {
        if (cameraCallback != null) {
            cameraCallback.onPreviewFrame(bytes, camera);
        }
    }
    ...
    public interface PreviewReadyCallback {
        public void onPreviewReady(int width, int height);
    }
...
//        this.getHolder().addCallback(this);
    ...
    public CameraPreview(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mLayoutMode = CameraPreview.LayoutMode.FitToParent;
        mHolder = getHolder();
    ...
    private void doSurfaceChanged(int width, int height) {
        ...
        if (null != mPreviewReadyCallback) {
            mPreviewReadyCallback.onPreviewReady(mPreviewSize.width, mPreviewSize.height);
        }
    }
...
//        float deltaRatioMin = Float.MAX_VALUE;
        int min = reqPreviewWidth + reqPreviewHeight;
        Camera.Size retSize = null;
        for (Camera.Size size : mPreviewSizeList) {
            int absW = Math.abs(size.width - reqPreviewWidth);
            int absH = Math.abs(size.height - reqPreviewHeight);
            if (min > (absW + absH)) {
                min = absH + absW;
                retSize = size;
            }
